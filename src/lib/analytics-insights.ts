import { AnalyticsData, ReferrerAnalysis, FunnelData } from "../types/analytics";
import { analyzeReferrer, getLineConversions } from "./supabase";
...
export const analyzeSourcePerformance = (
  ...
  userSessions.forEach(sessionData => {
    ...
    currentSourceData.exitPageCounts.set(exitPage, (currentSourceData.exitPageCounts.get(exitPage) || 0) + 1);
  });
  ...
  lineConversions.forEach(conversion => {
    ...
    if (sourceMap.has(source)) {
      ...
      currentSourceData.conversionPageCounts.set(conversionPage, (currentSourceData.conversionPageCounts.get(conversionPage) || 0) + 1);
    }
  });
  ...
  sourceMap.forEach((data, source) => {
    ...
    // 主要コンバージョンページをソート
    const sortedConversionPages = Array.from(data.conversionPageCounts.entries())
      .map(([page, count]) => ({ page, count }))
      .sort((a, b) => b.count - a.count);

    const conversionRate = data.inflowCount > 0 ? (data.conversionCount / data.inflowCount) * 100 : 0;
